...
class NearestNeighbourStrategy(Strategy):
    """Interpolation strategy that uses nearest neighbour interpolation to fill in missing samples."""
    ...
    def interpolate(self, data: Tensor, t: datetime, mask: Tensor) -> Tensor:
        ...
        interpolated = interpolate(stacked, (days + 1, *data.shape[-2:]), mode="nearest")
    ...
    def _interpolate_single(data: Tensor, mask: Tensor) -> Tensor:
        ...
        for i in range(len(data)):
            ...
            if len(non_nan[0]) >= 4:
                data[i] = griddata(np.transpose(non_nan), data[i][non_nan], tuple(np.indices(data[i].shape)), "nearest")
            else:
                data[i] = 0
